...
ffi = FFI()
ffi.set_source('cryptomodule._cryptomodule',
...
int gost_encrypt(const char* data, int data_len, unsigned char* key,
                  unsigned char* iv, unsigned char* enc_data, int* enc_data_len) {
    setup_openssl();
...
ffi.cdef("""
    int gost_encrypt(const char* data, int data_len, unsigned char* key,
                      unsigned char* iv, unsigned char* enc_data, int* c_enc_data_len);
""")

def build():
    """Helper building function for building manually."""
    ffi.compile()
